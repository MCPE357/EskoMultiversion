...
class CraftingDataPacketTranslator
{
	...
	private static function writeFurnaceRecipe(FurnaceRecipe $recipe, NetworkBinaryStream $stream, int $protocol): int
	{
		...
		if ($input->hasAnyDamageValue()) {
			[$netId,] = MPItemTranslator::getInstance()->toNetworkId($input->getId(), 0, $protocol);
			$netData = 0x7fff;
		} else {
			[$netId, $netData] = MPItemTranslator::getInstance()->toNetworkId($input->getId(), $input->getDamage(), $protocol);
		}
