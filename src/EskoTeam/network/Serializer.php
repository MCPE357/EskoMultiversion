...
use EskoTeam\misc\MPItemTranslator;
...
class Serializer
{
	...
	public static function putItemStack(NetworkBinaryStream $packet, int $protocol, Item $item, callable $writeExtraCrapInTheMiddle)
	{
		...
		[$netId, $netData] = MPItemTranslator::getInstance()->toNetworkId($item->getId(), $coreData, $protocol);
		...
		if ($isBlockItem) {
			...
			if ($block->getId() !== BlockIds::AIR) {
				$blockRuntimeId = MPRuntimeBlockMapping::toStaticRuntimeId($block->getId(), $block->getDamage(), $protocol);
			}
		}
		...
		$packet->putString(
			(static function () use ($protocol, $nbt, $netId): string {
				...
				if ($netId === MPItemTypeDictionary::getInstance()->fromStringId("minecraft:shield", $protocol)) {
	...
	public static function putRecipeIngredient(NetworkBinaryStream $packet, Item $item, int $protocol)
	{
		if ($item->isNull()) {
			...
			if ($item->hasAnyDamageValue()) {
				[$netId,] = MPItemTranslator::getInstance()->toNetworkId($item->getId(), 0, $protocol);
				$netData = 0x7fff;
			} else {
				[$netId, $netData] = MPItemTranslator::getInstance()->toNetworkId($item->getId(), $item->getDamage(), $protocol);
			}
			$packet->putVarInt($netId);
	...
	public static function getItemStack(DataPacket $packet, \Closure $readExtraCrapInTheMiddle, int $protocol): Item
	{
		...
		$null = null;
		[$id, $meta] = MPItemTranslator::getInstance()->fromNetworkId($netId, $netData, $null, $protocol);
	...
	public static function putGameRules(DataPacket $packet, array $rules, int $protocol)
	{
		...
		foreach ($rules as $name => $rule) {
			...
			if ($protocol >= ProtocolInfo::BEDROCK_1_17_0) {
